...
const PostEditorContent = ({ form, setQuillRef, onImageUpload,}) => {
  ...
  // Watch form values
   ...
   // Quill modules with custom image handler
    ...
    // Function for generating blog content
      const hasTitle = watchedValues.title?.trim();
      const hasContent =
      watchedValues.content && watchedValues.content !== "<p><br></p>";


      // Function for handling Ai content generation
      const handleAI = async(type, improvementType = null)=>{
        const { title, content, category, tags } = watchedValues;
      
      // Validate before generating or improving content
        if (type === "generate") {
          if (!title?.trim()) 
            return toast.error("Please add a title before generating the content! 😒")
      ...
      <div className='space-y-5'>
        ...
        <div>
          ...
          <div>{!hasContent ? (
            ...
            {!hasTitle && (
              <p className="text-md text-slate-400 w-full pt-2">
